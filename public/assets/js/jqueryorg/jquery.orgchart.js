(function($) {
    ...
    function OrgChart($container, opts){
        ...
        this.commitData = function(id,data){
            nodes[id].data.id = data.id;
            nodes[id].data.parent = data.parent;
            nodes[id].data.name = data.name;
            self.draw();            
        }

        this.startEdit = function(id, title = "Pilih "){
            ...
            $container.find('div[node-id='+id+'] h2').text(title);
            var commitChange = function(){
                var h2Element = $('<h2 id="title-node" node-id="'+id+'" class="title-node" parent_id="'+nodes[id].data.parent+'" onclick="Struktur.showDataKaryawan(this)">'+nodes[id].data.name+'</h2>');
                if(opts.allowEdit){
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.name !== 'undefined'){
                nameString = '<h2 id="title-node" node-id="'+self.data.id+'" parent_id="'+self.data.parent+'" class="title-node" onclick="Struktur.showDataKaryawan(this)">'+self.data.name+'</h2>';
            }
